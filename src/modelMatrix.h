...
public:
   ...
   void resid_correct(size_t col) {
      ...
      for (size_t obs=0; obs < F->data.nelem; obs++)
         residModel->resid[obs] -= regcoeff[col] * colptr[obsIndex[obs]];
   }
   ...
   void collect_lhs_rhs(size_t col) {
      ...
      for (size_t obs=0; obs < F->data.nelem; obs++) {
         ...
         rhs += colptr[matrixrow] * residModel->residPrec[obs] * residModel->resid[obs];
         lhs += colptr[matrixrow] * colptr[matrixrow] * residModel->residPrec[obs];
      }
   }
