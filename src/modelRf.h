   ...
   modelRf(std::string modelTerm, Rcpp::DataFrame &d, simpleMatrix &e, size_t resp)
      ...
      for(size_t i=0; i<nLevel1; i++) {         // ... and redo these labels based on matrix
         for(size_t j=0; j<nLevel2; j++) {      // names and order
            parLabels[i*nLevel2+j]= M1->labels[i] + "%" + M2->labels[j];
         }
      }
      ...
      nEvalUsed++;
      Rcpp::Rcout << "nEvalUsed first counting " << nEvalUsed << "\n";
...
//         Rcpp::Rcout << std::endl;
   ...
   void prepForOutput() {
      // solutions are made for all combinations, even when they don't exist in the data
