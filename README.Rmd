...
Currently, you need to install BayzR from github, which involves compiling the
package from source on your local machine. We are working on also getting pre-compiled
binary packages for Windows 10 and Mac-OS ready.

For all common users installation from the ljanss/BayzR repository is recommended,
this repository contains regulalry 'frozen' version that are tested on different
data sets and compile-tested on Windows Mac-OS. There is also a version in 
MarniTausen/BayzR, which is our development version and when downloading this you may
run into bugs or compilation errors. 
...
```
You will be asked to install several other packages, among other Rcpp. Accept to install these as well.
...
4). To use the summary() method with output from the bayz() function, you need
the coda-package:
```{r, eval=FALSE}
