...
#' shrinkage models, is specified with a hierachical model specification as Yield ~
...
#' V= option within the model-term function, for example Yield ~ rn(Variety, V=kinmat).
#' When the fit is for an interaction of factors, also the variance specification should
#' expand to include one term for each variable, separated by stars. The variance structure
#' is then built up from a list of given (proportional) variance-covariance or correlation
#' structures and from predefined acronyms IDEN, DIAG and VCOV to imply variance-covariance
#' structures with unknown (to be estimated from the data) parameters. 
#' for instance a fit of Location-Variety interaction effects could be 
#' Yield ~ rn(Location:Variety, V=loccor*kinmat), where 'loccor' is a matrix with known
#' correlations between locations. As explained above, 'loccor' should have row-names
#' matching the Locations in the data. Apart from known correlation-structures also 
#' special 
#'
